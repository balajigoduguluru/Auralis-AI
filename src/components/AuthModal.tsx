    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/90 backdrop-blur-2xl"
          role="dialog"
          aria-modal="true"
          aria-label="Satellite authentication"
        >
          <div className="max-w-md w-full bg-bg border border-white/20 rounded-[3rem] p-12 text-center space-y-12 shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <button
              onClick={onAbort}
              className="text-[10px] uppercase font-bold text-text-muted hover:text-error transition-colors"
